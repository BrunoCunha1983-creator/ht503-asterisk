...
export interface User {
    ...
    extension: string;
    password: string;
    /** Optional complete SIP address, for legacy Lovelace card compatibility. */
    aor?: string;
    /** Optional digest-auth username when it differs from the extension. */
    authorization_username?: string;
}

export interface LegacyCardClient {
    aor: string;
    username: string;
    password: string;
    name?: string;
...
export class SIPCore {
    ...
    private cardConfigFingerprint: string | null = null;
    ...
    get registered(): boolean {
        return Boolean(this.ua && this.ua.isRegistered());
    }
    ...
    private async setupAudio() {
        ...
        let audioElement = document.getElementById("remoteAudio") as HTMLAudioElement | null;
        if (!audioElement) {
    ...
    private async fetchConfig(hass: any): Promise<SIPCoreConfig> {
        ...
        if (resp.ok) {
            ...
            // Never print this object: it contains the current SIP credential.
            console.debug("SIP-Core configuration fetched");
            return config;
        } else {
    ...
    /**
     * Accept the configuration shape used by the original sipjs-client-card.
     * This lets an existing Lovelace block migrate by changing only the card type.
     * SIP Core still remains a singleton: one browser tab registers one selected client.
     */
    async configureFromLegacyCard(cardConfig: LegacyCardConfig): Promise<void> {
        const clients = cardConfig.clients || {};
        const currentUserId = this.hass?.user?.id;
        const currentUserName = this.hass?.user?.name;
        const selectedKey =
            cardConfig.client ||
            (currentUserId && clients[currentUserId] ? currentUserId : undefined) ||
            (currentUserName && clients[currentUserName] ? currentUserName : undefined) ||
            Object.keys(clients)[0];
        const selected = selectedKey ? clients[selectedKey] : undefined;

        if (!cardConfig.server || !selected?.aor || !selected?.username || !selected?.password) {
            throw new Error("Legacy card configuration requires server and one complete client entry");
        }

        const normalizedServer = this.normalizeWebSocketUrl(cardConfig.server);
        const normalizedAor = selected.aor.startsWith("sip:") ? selected.aor : `sip:${selected.aor}`;
        const aorWithoutScheme = normalizedAor.replace(/^sips?:/i, "");
        const [aorUser, aorHost] = aorWithoutScheme.split("@");
        const fingerprint = JSON.stringify({
            ...
            ringtone: cardConfig.ringtone,
            ringbacktone: cardConfig.ringbacktone,
            auto_answer: cardConfig.auto_answer,
        });

        if (fingerprint === this.cardConfigFingerprint) return;
        this.cardConfigFingerprint = fingerprint;

        if (this.ua) this.ua.stop();

        const fallbackConfig: SIPCoreConfig = {
            ice_config: {
            ...
            sip_video: false,
            pbx_server: aorHost,
            custom_wss_url: normalizedServer,
        };
        ...
        this.config = {
            ...
            ...(this.config || {}),
            custom_wss_url: normalizedServer,
            pbx_server: aorHost || this.config?.pbx_server || window.location.host,
            incomingRingtoneUrl: cardConfig.ringtone || this.config?.incomingRingtoneUrl || fallbackConfig.incomingRingtoneUrl,
            outgoingRingtoneUrl:
                cardConfig.ringbacktone || cardConfig.ringtone || this.config?.outgoingRingtoneUrl || fallbackConfig.outgoingRingtoneUrl,
            auto_answer: cardConfig.auto_answer ?? this.config?.auto_answer ?? false,
        };
        this.user = {
            ha_username: selectedKey || currentUserName || "lovelace",
            display_name: selected.name || currentUserName || selected.username,
            extension: aorUser || selected.username,
            password: selected.password,
            aor: normalizedAor,
            authorization_username: selected.username,
        };

        this.wssUrl = normalizedServer;
        await this.setupAudio();
        this.ua = this.setupUA();
        this.ua.start();
        this.triggerUpdate();
    ...
    /**
     * Accept host:port, ws://host:port or a complete WebSocket URL.
     * Asterisk normally publishes SIP WebSocket on /ws.
     */
    private normalizeWebSocketUrl(server: string): string {
        const withScheme = /^wss?:\/\//i.test(server.trim()) ? server.trim() : `wss://${server.trim()}`;
    ...
    private setupUA(): UA {
        ...
        const ua = new UA({
            sockets: [socket],
            uri: this.user.aor || `${this.user.extension}@${this.config.pbx_server || window.location.host}`,
            authorization_user: this.user.authorization_username || this.user.extension,
